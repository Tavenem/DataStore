...
{
    private Dictionary<string, IIdItem> _data = [];
    /// <summary>
    /// The stored data.
    /// </summary>
    ...
    /// </para>
    /// <para>
    /// This parameter is ignored for <see cref="InMemoryDataStore"/>; this method calls <see
    /// cref="GetItem{T}(string?, TimeSpan?)"/>.
    /// </para>
    /// </param>
    /// <param name="cacheTimeout">
    ...
    /// </returns>
    /// <remarks>
    /// This method wraps <see cref="GetItem{T}(string?, TimeSpan?)"/> in a completed <see
    /// cref="ValueTask"/>, since <see cref="InMemoryDataStore"/> is not asynchronous.
    /// </remarks>
    ...
    /// cref="ValueTask"/>, since <see cref="InMemoryDataStore"/> is not asynchronous.
    /// </remarks>
    public ValueTask<T?> GetItemAsync<T>(string? id, JsonTypeInfo<T>? typeInfo, TimeSpan? cacheTimeout = null) where T : class, IIdItem => new(GetItem<T>(id));
    ...
    public IDataStoreQueryable<T> Query<T>(JsonTypeInfo<T>? typeInfo = null) where T : class, IIdItem => new InMemoryDataStoreQueryable<T>(Data.Values.OfType<T>());
    ...
    /// </returns>
    /// <remarks>
    /// This method wraps <see cref="RemoveItem{T}(string?)"/> in a completed <see
    /// cref="ValueTask"/>, since <see cref="InMemoryDataStore"/> is not asynchronous.
    /// </remarks>
    ...
    /// </returns>
    /// <remarks>
    /// This method wraps <see cref="RemoveItem{T}(T)"/> in a completed <see cref="ValueTask"/>,
    /// since <see cref="InMemoryDataStore"/> is not asynchronous.
    /// </remarks>
    ...
    /// </para>
    /// <para>
    /// This method wraps <see cref="StoreItem{T}(T, TimeSpan?)"/> in a completed <see
    /// cref="ValueTask"/>, since <see cref="InMemoryDataStore"/> is not asynchronous.
    /// </para>
    /// </remarks>
    ...
    /// </para>
    /// </remarks>
    public Task<bool> StoreItemAsync<T>(T? item, JsonTypeInfo<T>? typeInfo, TimeSpan? cacheTimeout = null) where T : class, IIdItem => Task.FromResult(StoreItem(item));
}
